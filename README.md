...
- **早鸟价**：活动可配「早鸟价 + 前 N 名名额」，**非会员**前 N 名享早鸟价，会员不受影响。名额按占位口径计、行锁内重算防超发；后台区间校验「会员价 < 早鸟价 < 原价」。详情页「原价 + 🔥早鸟价·仅剩 N 名」
- **退改**：活动一旦开始即不可取消报名 / 退费（后端兜底）；取消文案按**实付**区分（会员免费报名「不退款、名额返还」，付费按实付退）；详情页地点行含详细地址 + 点击跳场馆页
- 后台报名列表展示每人**支付金额 + 命中的价格策略**；用户管理可查每人每笔订单

**分享**
- 发现 / 专栏 / 分类 / 场馆 / 会员 / 活动详情页支持转发好友 + 朋友圈
- 分享卡片图按场景适配：消息 5:4 logo 居中、朋友圈 1:1 占满；活动详情用封面（WebP→JPG 端点，微信分享卡片不渲染 WebP）

**合规（微信审核）**
- 登录弹窗去「微信」字样与官方视觉、品牌金按钮 + OFFENBAR logo；**主动勾选**同意《用户协议》《隐私政策》方可登录
- 独立协议页（`pages/agreement`）+ 微信隐私授权接口；全端去 CreatorOS → 敞开酒馆；我的订单（订单中心页 `pages/order`，支付合规）

**运营后台**
- **数据概览**：营收（累计 / 月 / 今日 + 环比）/ 会员（有效 + 到期提醒）/ 订单 / 价格策略分布 / 14 天趋势 / 用户状态分布 / 运维预警（纯 CSS 图表，不引图表库）
- **用户管理**：头像点击看大图、标签列显示自定义标签、搜索（用户 / 标签 / 状态字段）+ 会员筛选
- 活动 / 用户列表分页可选每页条数（10/15/20/30/100）

**运维 / 监控**
- 活动开始提醒（报名授权 + 开始前 2h 订阅消息）；用户状态收集（首次引导多选）
- 异常落库（`SystemLog`：接口 5xx / cron 失败 / 资源告警）+ 后台 **「服务健康」实时卡片**（API / 数据库 / 磁盘 / 内存 / 运行时长，30 秒刷新）
- 稳定性：开机自启全 enabled、PM2 日志轮转、每天 2:00 数据库备份（见 [`memory`]）；**GitHub Actions 外部健康巡检**每 5 分钟探测 `/api/health`，宕机邮件告警

**体验**
- 未设头像 / 昵称的用户：默认笑脸头像 + 默认昵称 momo（后台保留「未设置」便于识别）
...
- [x] 用户报名通知（活动开始前订阅消息）；用户状态收集
- [x] 报名退改规则（开始后不可取消 / 退款，文案按实付区分）；活动详情地点详细地址 + 跳场馆页
- [x] 全站分享（发现 / 专栏 / 分类 / 场馆 / 会员 / 详情）+ 分享卡片图按场景适配；默认头像 + momo
- [x] **数据概览升级**（营收 / 会员 / 订单 / 价格策略 / 趋势 / 预警）；用户管理搜索筛选 / 头像大图 / 自定义标签 / 分页
- [x] **运维监控**：服务健康实时卡片 + GitHub Actions 外部健康巡检（宕机邮件告警）；开机自启 + PM2 日志轮转

**待办**
- [x] ~~小程序合规整改后重新上传~~（已提交审核，2026-06）
- [ ] 小程序审核通过后**发布**上线
- [ ] 订单中心 path：`setorderpathinfo` 接口对非交易组件小程序不可用（40066），暂放下，不影响运营（见 `memory/wechat-review-compliance`）

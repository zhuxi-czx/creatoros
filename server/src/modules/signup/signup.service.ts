...
export class SignupService {
  ...
  async signupForEvent(userId: string, eventId: string) {
    ...
    const existingSignup = await this.prisma.signup.findUnique({
      where: {
        userId_eventId: {
          userId,
          eventId,
    ...
    if (existingSignup) {
      ...
      const signup = await this.prisma.signup.update({
        where: { id: existingSignup.id },
        data: { status: 'CONFIRMED' },
      });

      // Check if event is now full and update status
      await this.checkAndUpdateEventCapacity(eventId, event.maxCapacity);
    ...
    // Create signup
    const signup = await this.prisma.signup.create({
      data: {
        userId,
        eventId,
        status: 'CONFIRMED',
